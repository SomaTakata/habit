...
gen-dbmodel: clean-dbmodel ## DBモデルを生成
	@xo schema mysql://$(RDB_USER):$(RDB_PASS)@$(RDB_HOST):$(RDB_PORT)/$(RDB_NAME) --out backend/db_model -e *.created_at -e *.updated_at --src backend/db_model/templates/go --go-import="habit/backend/pkg/tenant"
