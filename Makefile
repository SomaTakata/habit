...
DATABASE_HOST ?= "postgres://$(RDB_USER):$(RDB_PASS)@$(RDB_HOST):$(RDB_PORT)?sslmode=disable"
...
gen-dbmodel: clean-dbmodel ## DBモデルを生成
	@go run -mod=mod github.com/xo/xo schema postgres://$(RDB_USER):$(RDB_PASS)@$(RDB_HOST):$(RDB_PORT)/$(RDB_NAME)?sslmode=disable --out backend/db_model -e *.created_at -e *.updated_at --src backend/db_model/templates/go --go-import="habit/backend/pkg/tenant"
